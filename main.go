...
func main() {
	processTitle = ("Dart Gen - Accounts Created: " + strconv.Itoa(accountsCreated))
	title.SetTitle(processTitle)
	startUp()
	richPresence()
	...
	fmt.Println("How many tasks would you like to create?")
	...
	p, _ := ants.NewPool(thread, ants.WithExpiryDuration(10*time.Minute))
	defer p.Release()
	...
	for i := 1; i <= numberOfTasks; i++ {
		...
		_ = p.Submit(func() {
			syncProcess()
			if accountsCreated >= numberOfAccounts {
				p.Release()
				os.Exit(69)
			}
		})
...
func process() {
	...
	registerAccount := func() {
		...
		var proxyBan []*cdp.Node
		var login []*cdp.Node
		err = chromedp.Run(ctx,
			chromedp.Nodes(`/html/body/div[2]/div[3]/div[4]/div/div[2]/input`, &proxyBan, chromedp.AtLeast(0)))
		if len(proxyBan) == 0 {
			err = chromedp.Run(ctx,
				chromedp.Navigate(`https://www.nike.com/member/settings`),
				chromedp.Nodes(`/html/body/div[2]/div[3]/div[7]/form/div[6]/input`, &login, chromedp.AtLeast(0)))
			if len(login) == 0 {
				err = chromedp.Run(ctx,
				...
				if err != nil {
					...
					if provider == 1 {
						...
						for attemptCount := 1; attemptCount <= 21; attemptCount++ {
							if strings.Contains(phoneCode, "FULL_SMS") {
								...
								blue.Println("Code found:", phoneCode)
								err = chromedp.Run(ctx,
								...
								boldGreen.Println("Successfully registered Nike account!")
								break

							} else if attemptCount <= 20 && phoneCode == "STATUS_WAIT_CODE" {
								yellow := color.New(color.FgYellow)
								yellow.Println("Waiting 15 seconds. Attempt:", attemptCount)
								err = chromedp.Run(ctx, chromedp.Sleep(15*time.Second))
								phoneCode = getSmsActivate(smsActivateID)

							} else if attemptCount == 21 && phoneCode == "STATUS_WAIT_CODE" {
								cancelSmsActivate(smsActivateID)
								color.Red("SMS attempt limit reached. Phone number cancelled.")

							} else if phoneCode == "STATUS_CANCEL" {
								color.Red("Activation cancelled.")
							}
						}
					} else if provider == 2 {
